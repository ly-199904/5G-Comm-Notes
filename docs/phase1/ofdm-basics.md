...
| **Extended CP** | **仅 μ=2** | 16.67 μs | ~25% | 60 kHz SCS；大时延扩展场景（如城区大覆盖）|
...
$$
b_k = \frac{1+j}{\sqrt{2}} \cdot \{-1, +1\}^k \cdot e^{j\pi k/2}
$$

每个符号相位旋转 π/2，使得相邻符号的相位差固定，时域波形近似恒包络。这是 NTN 上行 PUCCH Format 3 选择 π/2-BPSK 的根本物理原因。

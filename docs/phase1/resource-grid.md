...
```
100 MHz 载波（275 RB @ 30 kHz SCS）

CRB: 0          30         80         150        200       274
      ↓          ↓          ↓          ↓          ↓         ↓
      ┊          ┊──Initial BWP (50 RB)──┊         ┊         ┊
      ┊          ┊                        ┊         ┊         ┊
      ┊     ┊──────────── Active BWP #1 (106 RB) ──────────┊  ┊
...
│  ↓  Point A = CRB#0 最低子载波（全局锚点，数学原点）                  │
│  │                                                                  │
│  ├──[ offsetToCarrier ]──→ 载波可用 RB 起点                         │
│  │   单位：当前 SCS 的 RB 数                                         │
│  │                                                                  │
│  ├──[ offsetToPointA ]──→ SSB 最低 RB（参考 SCS 的 RB 数）          │
│  │   └──[ k_SSB ]──→ SSB 最低子载波（子载波粒度）                   │
│  │                                                                  │
│  └──[ BWP locationAndBandwidth ]──→ BWP 起点 & 带宽                 │
│      解码：startRB = ⌊LAB/37⌋，nRB = (LAB mod 37) + 1              │
│                                                                     │
│  参考 SCS：FR1 = 15 kHz，FR2 = 60 kHz（与 BWP 实际 SCS 无关！）      │
└─────────────────────────────────────────────────────────────────────┘
```

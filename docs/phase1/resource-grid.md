...
| 3 | 120 kHz | 24 | **264** | **380 MHz** |
| 4 | 240 kHz | 24 | 138 | **397.44 MHz**（FR2 上限）|

> **FR2 RB 数上限**：μ=3（264 RB）和 μ=4（138 RB）均受 FR2 射频带宽上限（400 MHz）约束。
> 264 × 12 × 120 kHz = 380 MHz，138 × 12 × 240 kHz = 397 MHz，两者均在 400 MHz 以内。
...
#### 3.3 虚拟资源块 VRB 与资源分配类型

DCI 中指定的 RB 实际上是**虚拟资源块（VRB）**，最终还需映射到 PRB：

**Type 0 资源分配**（位图方式）：
- 以 **RBG（Resource Block Group）** 为粒度，每个 bit 代表一组 RB
- RBG 大小由 BWP 总 RB 数决定（38.214 Table 5.1.2.2.1-1）
- 适合非连续资源分配（频率选择性调度）

**Type 1 资源分配**（连续 RIV 方式）：
- 用 RIV（Resource Indication Value）编码连续 VRB 的起点和长度
- $\text{RIV} = N_\text{BWP}^\text{size}(L_{RB}-1) + R_B^\text{start}$（若 $L_{RB} \leq \lfloor N/2 \rfloor$）
- 适合连续资源分配（频率非选择性信道）
